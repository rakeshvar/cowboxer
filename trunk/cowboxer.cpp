    ...
    for(int x = x1; x <= x2; x++)
    {
        ...
        {   // An open box is closed when a vertical white line is found
            if (isVWhite(x, y1, y2)) {
                xclose = x;
                closed = true;
                trimAndAddBox(xopen, y1, xclose - xopen, y2-y1+1, str);
            }}
    }
    if (!closed)
        trimAndAddBox(xopen, y1, x2-xopen+1, y2-y1+1, str);

    if (currentBox > 0) currentBox--; // Set Current Box to the last created box

    MOD_REPAINT_EMIT;
}

void cowBoxer::trimAndAddBox(int x, int y, int w, int h, QString str)
{
    // Trims a box to remove white space at the top and bottom
    while (isHWhite(x, x+w-1, y))       { y++; h--;   }
    while (isHWhite(x, x+w-1, y+h-1))   {      h--;   }
